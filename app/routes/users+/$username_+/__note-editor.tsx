...
type NoteEditor = z.infer<typeof NoteEditorSchema>

const resolver = zodResolver(NoteEditorSchema)
...
export function NoteEditor({ note }: { note?: Info['loaderData']['note'] }) {
	...
	const { handleSubmit, watch, formState, register, setValue } = form
									...
									return (
										...
										>
											<button
												...
												onClick={() => setValue('images', imageList)}
												type="button"
											>
												<span aria-hidden>
													<Icon name="cross-1" />
 ...
 */
	const form = useRemixFormContext<NoteEditor>()
	const { fields } = useFieldArray({ name: 'images', control: form.control })
	const existingImage = Boolean(fields?.length)
	const [previewImage, setPreviewImage] = useState<string | null>(
		objectKey ? getNoteImgSrc(objectKey) : null,
	)
	const [altText, setAltText] = useState(fields?.[0]?.altText ?? '')
